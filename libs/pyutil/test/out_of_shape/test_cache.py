...
class Testy(unittest.TestCase):
    ...
    def _test_LRU_full(self, C):
        ...
        self.assertTrue(10 in list(c.values()), list(c.values()))
        self.assertTrue(0 not in list(c.values()))

        del c[1]
        c._assert_invariants()
        self.assertTrue(1 not in list(c.values()))
        self.assertTrue(len(c) == 9)
        c[11] = 11
        c._assert_invariants()
        self.assertTrue(len(c) == 10)
        self.assertTrue(1 not in list(c.values()))
        self.assertTrue(11 in list(c.values()))
        del c[11]
        c._assert_invariants()
        ...
        for i in range(200):
            c[i] = i
            c._assert_invariants()
        self.assertTrue(199 in list(c.values()))
        self.assertTrue(190 in list(c.values()))
    ...
    def _test_LRU_has_key(self, C):
        ...
        self.assertTrue(not 2 in list(c.values()))
    ...
    def _test_mem_leakage_much_adding_some_removing(self):
        ...
        self.assertTrue(slope <= MIN_SLOPE,
                        "%s leaks memory at a rate of approximately %s system bytes per invocation" % (
                        self._mem_test_LRU_much_adding_some_removing, "%0.3f" % slope,))

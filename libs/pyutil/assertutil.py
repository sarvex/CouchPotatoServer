...
def _assert(___cond=False, *___args, **___kwargs):
    ...
    if ___args:
        ...
        if ___kwargs:
            msgbuf.append(", %s: %s %s" % ((list(___kwargs.items())[0][0],) + tuple(
                map(hr, (list(___kwargs.items())[0][1], type(list(___kwargs.items())[0][1]),)))))
    else:
        if ___kwargs:
            msgbuf.append("%s: %s %s" % ((list(___kwargs.items())[0][0],) + tuple(
                map(hr, (list(___kwargs.items())[0][1], type(list(___kwargs.items())[0][1]),)))))
    msgbuf.extend([", %s: %s %s" % tuple(map(hr, (k, v, type(v),))) for k, v in list(___kwargs.items())[1:]])

...
def collect_parameters(uri_query='', body=[], headers=None,
    ...
    if uri_query:
        params.extend(urllib.parse.parse_qsl(uri_query, keep_blank_values=True))
    ...
    if headers:
        headers_lower = dict((k.lower(), v) for k, v in list(headers.items()))
    ...
    if exclude_oauth_signature:
        unescaped_params = [i for i in unescaped_params if i[0] != 'oauth_signature']

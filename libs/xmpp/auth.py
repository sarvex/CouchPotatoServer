...
class NonSASL(PlugIn):
    ...
    def plugin(self,owner):
        ...
        if query.getTag('digest'):
            ...
            for foo in range(int(seq)): hash = sha.new(hash).hexdigest()
...
class SASL(PlugIn):
    ...
    def plugout(self):
        ...
        if 'features' in self._owner.__dict__: self._owner.UnregisterHandler('features', self.FeaturesHandler,
                                                                             xmlns=NS_STREAMS)
        if 'challenge' in self._owner.__dict__: self._owner.UnregisterHandler('challenge', self.SASLHandler,
                                                                              xmlns=NS_SASL)

...
class CommonClient:
    ...
    def reconnectAndReauth(self):
        ...
        if 'ComponentBind' in self.__dict__: self.ComponentBind.PlugOut()
        if 'Bind' in self.__dict__: self.Bind.PlugOut()
        self._route=0
    ...
    def connect(self,server=None,proxy=None,ssl=None,use_srv=None):
        ...
        if 'version' in self.Dispatcher.Stream._document_attrs and self.Dispatcher.Stream._document_attrs[
            'version'] == '1.0':
...
class Client(CommonClient):
    ...
    def connect(self,server=None,proxy=None,secure=None,use_srv=True):
        ...
        if not CommonClient.connect(self, server, proxy, secure,
                                    use_srv) or secure != None and not secure: return self.connected
        transports.TLS().PlugIn(self)
        if 'version' not in self.Dispatcher.Stream._document_attrs or not self.Dispatcher.Stream._document_attrs[
                                                                              'version'] == '1.0': return self.connected
        while not self.Dispatcher.Stream.features and self.Process(1): pass      # If we get version 1.0 stream the features tag MUST BE presented

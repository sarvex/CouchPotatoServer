...
class Automation(Plugin):
    ...
    def setCrons(self):
        fire_event('schedule.interval', 'automation.add_movies', self.addMovies, hours=self.conf('hour', default=12))

    def addMovies(self):

        movies = fire_event('automation.get_movies', merge=True)
        movie_ids = []
        ...
        for imdb_id in movies:
            ...
            if not added:
                added_movie = fire_event('movie.add', params={'identifier': imdb_id}, force_readd=False,
                                         search_after=False, update_after=True, single=True)
                if added_movie:
        ...
        for movie_id in movie_ids:
            ...
            movie_dict = fire_event('media.get', movie_id, single=True)
            if movie_dict:
                fire_event('movie.searcher.single', movie_dict)
